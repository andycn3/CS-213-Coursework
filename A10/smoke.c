...
void* wait_match(void* av) {
  ...
  uthread_mutex_lock(a->mutex);
	...
	while (1) {
		...
		if (ingreds_avail[PAPER] + ingreds_avail[TOBACCO] == 0) {
			ingreds_avail[MATCH] = 1;
		} else {
			smoke_count[ingreds_avail[PAPER] == 1 ? TOBACCO : PAPER]++; 
			ingreds_avail[ingreds_avail[PAPER] == 1 ? PAPER : TOBACCO] = 0;
...
void* wait_paper(void* av) {
  ...
  uthread_mutex_lock(a->mutex);
	...
	while (1) {
		...
		if (ingreds_avail[MATCH] + ingreds_avail[TOBACCO] == 0) {
			ingreds_avail[PAPER] = 1;
		} else {
			smoke_count[ingreds_avail[MATCH] == 1 ? TOBACCO : MATCH]++; 
			ingreds_avail[ingreds_avail[MATCH] == 1 ? MATCH : TOBACCO] = 0;
			uthread_cond_signal(a->smoke);
		}
	}
  uthread_mutex_unlock(a->mutex);
...
void* wait_tobacco(void* av) {
  ...
  uthread_mutex_lock(a->mutex);
	...
	while (1) {
		...
		if (ingreds_avail[MATCH] + ingreds_avail[PAPER] == 0) { 
			ingreds_avail[TOBACCO] = 1;
		} else { 
			smoke_count[ingreds_avail[PAPER] == 1 ? MATCH : PAPER]++; 
			ingreds_avail[ingreds_avail[PAPER] == 1 ? PAPER : MATCH] = 0; 
			uthread_cond_signal(a->smoke);
		}
	}
  uthread_mutex_unlock(a->mutex);
...
int main (int argc, char** argv) {
  ...
  uthread_t smoker_threads[3];

  uthread_init(5);
  
  void* (*proc[3])(void*) = {wait_match, wait_paper, wait_tobacco};
  for (int i = 0; i < 3; i++)
    smoker_threads[i] = uthread_create(proc[i], a);
